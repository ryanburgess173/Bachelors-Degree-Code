...
//        product.getParts().addAll(assparts);
            ...
            if(product.getId()!=0) {
                ...
                if(product.getInv()- product2.getInv()>0) {
                    for (Part p : product2.getParts()) {
                        int inv = p.getInv();
                        if(inv - (product.getInv() - product2.getInv()) < p.getInvMin()) {
                        	return "PartMinimumNotMet";
                        }
    ...
    public String buyProduct(@RequestParam("productID") int theId, Model theModel) {
    	...
    	if(inventoryCount>0) {
        	product.setInv(inventoryCount-1);
        	productService.save(product);
        	return "confirmationProductPurchased";
    	}else {
    		return "errorMakingPurchase";
    	}
    }
